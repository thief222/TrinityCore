...
void CreatureTextMgr::SendSound(Creature* source, uint32 sound, CreatureTextSoundType soundType, ChatMsg msgType, WorldObject const* whisperTarget, CreatureTextRange range, Team team, bool gmOnly)
{
    ...
    switch (soundType)
    {
        case CreatureTextSoundType::DirectSound:
            data = *WorldPackets::Misc::PlaySound(source->GetGUID(), sound).GetRawPacket();
            break;
        case CreatureTextSoundType::ObjectSound:
            data = *WorldPackets::Misc::PlayObjectSound(source->GetGUID(), whisperTarget ? whisperTarget->GetGUID() : source->GetGUID(), sound).GetRawPacket();
            break;
        case CreatureTextSoundType::Music:
            data = *WorldPackets::Misc::PlayMusic(sound, source->GetGUID()).GetRawPacket();
            break;
        default:
            break;
    }
